...
function artifactKindExecutionGuidance(kind: ArtifactKind, language: string): string {
  ...
  const blocks: Record<ArtifactKind, string> = {
    ...
    landing: [
      ...
      "- Tailwind: `className` only (CDN). **No** `*.module.css`/CSS Modules — превью Lemnity их не поддерживает; только Tailwind или редкий глобальный `src/index.css`.",
    ...
    lovable: [
      ...
      copyNote,
      "- Styling: Tailwind utility `className` (preview shell injects Tailwind CDN; no PostCSS). **Forbidden:** CSS Modules (`*.module.css`, `import styles from \"./X.module.css\"`) — они ломают esbuild-превью Lemnity; только Tailwind + при необходимости глобальный `src/index.css`.",
...
export function executeLovableUiPrompt(input: {
  ...
  return [
    ...
    "- Use Tailwind classes in `className` only (Tailwind CDN is injected in preview). Never import `*.module.css`; use Tailwind instead.",
