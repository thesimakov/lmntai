...
/** Подставляется в поле чата / сборки при выборе шаблона — пользователь правит и отправляет. */
export const MASSAGE_DEFAULT_USER_PROMPT = `Сайт мастера массажа на базе шаблона, стиль — минимализм.

Сделай:
- Название студии, слоган, мои контакты (телефон, адрес, часы) — подставь плейсхолдеры, если нет данных.
- Разделы: услуги с ценами, обо мне, запись (кнопка/форма-заглушка), футер.
- Мультистраничность: отдельные «экраны» или якоря — услуги, мастер, запись, политика.
- Роли: гость (видит публичное) и клиент (после «входа» — история визитов, запись) — в UI как демо-переключатель.

Правь тексты в существующих src/App.tsx / компонентах шаблона, сохрани структуру.`;
