  ...
  return {
    ...
    setup(build) {
      build.onResolve({ filter: /.*/ }, (args) => {
        ...
        try {
          return { path: reqFromAppRoot.resolve(args.path) };
        } catch {
          return undefined;
        }
      });
...
/**
 * Статичная страница превью, если esbuild не смог собрать (неизвестный импорт, синтаксис…).
 * Не подставляем весь сырой ответ модели — только краткое сообщение.
 */
...
/**
 * Собирает HTML с inline ESM (React в бандле).
 */
