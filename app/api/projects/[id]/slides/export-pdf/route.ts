...
import { buildSlideGraphPdf } from "@/lib/slide-graph/pdf-export";
...
export async function POST(
  ...
  try {
    pdfBuffer = buildSlideGraphPdf(graph);
  } catch (err) {
    console.error("[slides/export-pdf] PDF generation failed:", err instanceof Error ? err.message : err);
    return apiError("Failed to generate PDF file.", 500);
  }

  const filename = `${graph.meta.title.replace(/[^a-zA-Z0-9а-яА-Я\s_-]/g, "").trim() || "presentation"}.pdf`;
