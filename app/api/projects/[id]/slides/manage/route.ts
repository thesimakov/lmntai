...
export async function POST(
  ...
  const graphParse = slideGraphSchema.safeParse(JSON.parse(state.files["slide_graph.json"]));
  ...
  if (op.op === "add") {
    const newSlide = await generateNewSlide(graph, op.prompt ?? "", user.id, projectId);
...
Schema:
{
  "id": "slide_X",  // unique, not in [${existingIds}]
  "layout": "content" | "title" | "two-column" | "image-left" | "image-right" | "blank" | "quote" | "section-divider",
  "background": { "color": "#optional-hex" },
  "elements": [
    { "id": "elem_X", "type": "heading"|"subheading"|"body"|"bullet-list"|"quote"|"caption"|"label", "content": "...", "items": ["..."] }
  ],
  ...
  try {
    ...
    const parsed = JSON.parse(json) as Partial<Slide>;
  ...
  return {
    ...
    layout: "content",
    elements: [
      { id: `${id}_h`, type: "heading", content: hint || "New Slide" },
      { id: `${id}_b`, type: "body", content: "Add your content here." },
    ],
  };
}
