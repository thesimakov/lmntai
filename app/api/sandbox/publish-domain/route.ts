...
async function withOwner(
  req: NextRequest,
  work: (arg: { projectId: string; user: { id: string; plan: string } }) => Promise<Response>
): Promise<Response> {
...
async function getPublishDomains(req: NextRequest) {
  return withOwner(req, async ({ projectId, user }) => {
    const rows = await listPublishHostsForSandbox(user.id, projectId);
...
async function postPublishDomain(req: NextRequest) {
  return withOwner(req, async ({ projectId, user }) => {
    ...
    await setSandboxSharePublic(projectId, user.id, true);
    return Response.json(
      {
        host: bound.host,
...
async function deletePublishDomain(req: NextRequest) {
...
async function putPublishDomain(req: NextRequest) {
  return withOwner(req, async ({ user }) => {
    const body = (await req.json().catch(() => null)) as { host?: string } | null;
    const hostRaw = body?.host?.trim() ?? "";
    if (!hostRaw) return new Response("host is required", { status: 400 });
...
export const GET = withApiLogging("/api/sandbox/publish-domain", getPublishDomains);
export const POST = withApiLogging("/api/sandbox/publish-domain", postPublishDomain);
export const DELETE = withApiLogging("/api/sandbox/publish-domain", deletePublishDomain);
export const PUT = withApiLogging("/api/sandbox/publish-domain", putPublishDomain);
